...
description: "LI.FI intent has two ways of emitting orders: 1. On-chain through the deposit interface or through the order server. For ease of integration and to ensure you receive all orders, it is recommended that you connect to the order server for events."
sidebar:
  order: 2
  badge:
    text: Testnet!
...
LI.FI intent allows highly customizable orders. As a result, you want to add further validation to ensure that you support the order being relayed to you. As a result, it is important to properly validate orders. 
