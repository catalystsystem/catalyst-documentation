...
For instructions on how to relay proof for self-serve systems, please refer to their section in [validation](/architecture/oracle).
...
For the CompactSettler (and compliant settlers), you need to deliver the order as you originally received it in `StandardOrder`, the attached signatures as `bytes.concat(sponsor, allocator)`, the timestamps of the fills (which can be read from the OutputFilled event), and the solver's identifier or address. If there are multiple outputs, always index lists by their position in the order.
