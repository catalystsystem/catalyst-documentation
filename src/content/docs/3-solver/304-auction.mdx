...
description: "LI.FI intent supports various order types and auctions to get the best price for users. Solvers compete in these auctions through various means."
sidebar:
  order: 4
---

LI.FI intent supports three order types: Limit Order (First come, first serve / gas auction), Dutch Auction (on-chain), and off-chain auctions.
...
LI.FI intent Dutch auctions are slightly more complicated. The fulfillment context will have the slope and stopTime encoded.
...
Li.FI intent off-chain auctions allow for complex auction logic at the cost of centralization. An order can choose an off-chain entity to execute an auction on its behalf.
