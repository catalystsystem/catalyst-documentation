---
title: "Output Settlement"
slug: "architecture/output"
description: "LI.FI intent currently supports two order types: simple limit orders and Dutch auctions. However, the LI.FI intent was designed with flexibility in mind, and more can easily be added."
sidebar:
  order: 2
---

Currently only one output settler exists: – [`OutputSettlerCoin.sol`](https://github.com/openintentsframework/oif-contracts/blob/main/src/output/coin/OutputSettlerCoin.sol) – supporting four order types: simple limit orders, Dutch auctions, exclusive limit orders, and exclusive dutch auctions.. For more about the specific order details, see the section on [Auctions](/solver/auctions)

## CoinFiller

To configure which auction type used set the associated `output.context`.

import { Code } from '@astrojs/starlight/components';

export const mandateOutput = `
struct MandateOutput {
    ...
    bytes call;
    bytes context;
}`;

<Code code={mandateOutput} lang="solidity" mark={['bytes context;']} />

Use the following pseudocode:

```solidity
uint256 slope;
uint256 stopTime;
string orderType; // "limit" or "dutch"

if (orderType == "limit") fulfillmentContext = "0x" || fulfillmentContext = "0x00";
if (orderType == "dutch") fulfillmentContext = abi.encodePacked(0x01, slope, stopTime);
```

Specifically for Dutch auctions, if the order contains multiple outputs, only the first one will work as an auction, and the rest will resolve to the worst price. This is because solvers are only incentivized to compete on the first output in an order, since the winner of that order is the winner of the entire order once anyone fills the rest of the outputs.